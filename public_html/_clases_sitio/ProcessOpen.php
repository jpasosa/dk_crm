...
// Arma el array con todos los Procesos ABIERTOS | También pueden ser cerrados que levantan a otros formularios una vez cerrados.
	...
	{ // Comienza área 4 (ADMINISTRACIÓN).
		...
		//
		// Ahora tengo que buscar los tra_carga_mercaderia_transito cerrados. . .
		//
		$tra_carga_mercaderia_transito_cerrados = BDConsulta::consulta('tra_carga_mercaderia_transito_cerrados', array(), 'n');
		if(!is_null($tra_carga_mercaderia_transito_cerrados))
		{
			foreach($tra_carga_mercaderia_transito_cerrados AS $tplc)
			{
				$first_process_tcmt = Process::getFirstProcess('tra_carga_mercaderia_transito', $tplc['id_tra_carga_mercaderia_transito']); // agaroo el primer proceso.
				if(isset($first_process_tcmt) && !is_null($first_process_tcmt[0])) {
					$last_process_tpl = Process::getLastProcess('tra_carga_mercaderia_transito', $first_process_tcmt[0]['id_tra_carga_mercaderia_transito_proc']); // agaroo el ultimo proceso.
					$dias_activo = $last_process_tpl['dias_activo']; // guardo dias_activo
					$fecha_inicio = $last_process_tpl['fecha_alta']; // y guardo fecha_alta
				}
				if(!is_null($first_process_tcmt))
				{
					$days = Dates::CalculateDaysExpire($fecha_inicio, $dias_activo, '1');
					// Calculo [fecha_vence] => 19/06/2013  [pasaron_dias] => 18    [restan_dias] => 0   [prioridad] => ALTA    [alarma] => rojo
					if($days['alarma'] == 'verde')
					{   // procesos de TAREAS PENDIENTES (verde)
						array_push($all_process['verde']['terceros'],
													array('id_tabla_proc' 	=> $tplc['id_tra_carga_mercaderia_transito'],
														'proceso_proceso' 	=> 'tra_ytd_entrada',
														'proceso_nombre' 	=> 'Trafico |  YTD Entradas ' . '(' . $tplc['observaciones'] . ')',
														'fecha_inicio' 		=> $fecha_inicio,
														'fecha_vence' 		=> $days['fecha_vence'],
														'pasaron_dias' 		=> $days['pasaron_dias'],
														'restan_dias' 		=> $days['restan_dias'],
